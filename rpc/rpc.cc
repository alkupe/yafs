...
//for function object for finding the xid in the list and checking if it is forgotten
bool rpcs::findXid::operator()( reply_t rep )
{
        if (xid > rep.xid && earliest != NULL)
          *earliest = false;
        return rep.xid == xid;
}

//for doing cleanup, returns true for ones that should be removed and frees the buffer pointer
bool rpcs::cleanup::operator() ( reply_t rep ) const {
  if (rep.xid < xid_rep) {
    free(rep.buf);
    return true;
  } else {
		...
		unsigned int xid_rep, char **b, int *sz)
{
	ScopedLock rwl(&reply_window_m_);
	rpcstate_t myReturn;
	Replywindow::iterator iter;


	//we know that clt_nonce is in the map because earlier code checks and creates the entry if not
	iter = reply_window_.find(clt_nonce);
	VERIFY(iter != reply_window_.end());

  if (iter->second.size() == 0) {
    //the client list is empty, so it must be new:
    myReturn = NEW;
  } else {
  //list not empty so search the list
    bool tooEarly(true);
    findXid pred(xid,&tooEarly);
    std::list<reply_t>::iterator rep_iter;
    rep_iter = find_if(iter->second.begin(),iter->second.end(),pred);

    //if we couldn't find it, its either forgotten or new
    if (rep_iter == iter->second.end()) {
      //its xid is < the earliest in the list
      if (tooEarly) {
        myReturn = FORGOTTEN;
      } else {
        //its new
      ...
      //found it, its either in progress or done
      if (!rep_iter->cb_present) {
        myReturn = INPROGRESS;
      } else {
        //its done, so copy the results
        *b = rep_iter->buf;
        *sz = rep_iter->sz;
        myReturn = DONE;
      }
  ...
  if (myReturn == NEW) {
    reply_t new_rep(xid);
    new_rep.cb_present = false;
    iter->second.push_back(new_rep);
  }

  //do cleanup, removing unnecessary xids from the reply window
  cleanup cleanupPred(xid_rep);
  iter->second.erase(remove_if(iter->second.begin(),iter->second.end(),cleanupPred),iter->second.end());
