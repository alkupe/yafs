...
class rpcs : public chanmgr {
	...
	//function object for looking for an xid in a list of xids
	//also checks if the xid to be found is before the earliest xid in the list (xid < xid for all xids in the list)
	//if the find_if comes back at the end of the list && earliest is false, then the xid is forgotten
	struct findXid{
	  unsigned int xid;
	  bool* earliest;

	  findXid(unsigned int xidToFind,bool* tooEarly = NULL) : xid(xidToFind),earliest(tooEarly) {}
	  bool operator () ( reply_t rep );
	  };

	//function object for cleanup
	 struct cleanup{
	    unsigned int xid_rep;
	    cleanup(unsigned int xid_rep_curr) : xid_rep(xid_rep_curr) {}
	    bool operator () ( reply_t rep ) const ;
	   };
  typedef std::map<unsigned int, std::list<reply_t> > Replywindow;
	int port_;
	unsigned int nonce_;
